...
public class ComponentPG extends ComponentAPI implements Serializable {
    ...
    private List<VendorPG> vendors = new java.util.ArrayList<>();
    ...
    public void setVendors(List<VendorAPI> vendors) {
        this.vendors = vendors.stream().map(vendor -> {
            return new VendorPG(vendor);
        }).collect(Collectors.toList());
    }
    ...
    @Override
    public @Valid LocalDate getCreatedOn() {
